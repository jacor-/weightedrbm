...
class RBM(object):
    ...
    def _prepareCDK(self, k):
        ...
        trng = theano.tensor.shared_randomstreams.RandomStreams(self.SEED)
    ...
    def _prepareProbs(self):
        ...
        self.compute_prob_valid_states = theano.function([], self.probability_tensor[self.data_indexs])
                                
    def __init__(self, VISIBLES, HIDDEN, dataset, neuronType = 0, sigma = 0.1, k = 1, type = "equal_weights", require_probs = False):
        '''
        ...
        self.require_probs = require_probs
        self.SEED = 1234
        ...
        self.data = theano.shared(numpy.asarray(dataset[0], dtype=theano.config.floatX))  # @UndefinedVariable
        self.data_probabilities = theano.shared(numpy.asarray(dataset[1], dtype=theano.config.floatX).reshape(-1,1))  # @UndefinedVariable
 

        if self.type == "exact" or require_probs == True:
            ...
            for j in range(2**VISIBLES):
                ...
                for i in range(VISIBLES):
                    ...
                    valor = valor / 2
                    if neuronType == 1 and residuo == 0:
                        just_visible_units[j, VISIBLES-i-1] = -1
                    else:
            ...
            data_indexs = []
            for array_j in dataset[0]:
                valor = 0
                for i in range(VISIBLES):
                    valor = 2 * valor + (array_j[i]>0)
    ...
    def print_values(self):
        probs = self.getProbabilityValidStates()
        model_probs = self.data_probabilities.get_value()
        KL = numpy.sum(model_probs * numpy.log(model_probs/ probs))
        log_likelihood = -numpy.mean(numpy.log(probs))
        sum_probs = numpy.sum(probs)

        print("Train")
        print("  KL  %f  LogLKH  %.3f  SumProb  %.3f" % \
                (KL, log_likelihood, sum_probs))
    ...
    def print_probs(self):
        ...
        for i in range(len(model_probs)):
            print("\t%-16s  %f   %f" % (self.data.get_value()[i], probs[i], model_probs[i]))
        sum_probs = sum(probs)
        log_lkh = numpy.mean(numpy.log(probs))
        print("\tLogLKH_Data  %.3f  SumProb_Data:  %.3f" % (log_lkh, sum_probs))
        return
            
                        
    def train(self, hyperparameters, nPrint, File_w):
        ...
        if CDk != None:
            print "WARNING: (training RBM) Remember that CDk has been set during intialization... this parameter is not being taken into account here"
        if MiniBatchSize != None:
            print "WARNING: (training RBM) Minibatch option not implemented yet"
        if WeightDecay != None:
            print "WARNING: (training RBM) WeightDecay not implemented yet"



        for current_epoch in range(MinEpochs, MaxEpochs):
            self.trainStep(LRate, Momentum)
            #if self.require_probs:
            #    monit_likelihood.append(self.getLikelihood())
            
            # Print results every num_print epochs
            if (current_epoch + 1) % nPrint == 0:
                print("Epoch %5d" % (current_epoch + 1))
                _, train_KL = self.print_values()
                print("\n")
    ...
    def getProbabilityValidStates(self):
        '''
        Just call this function to perform a single training epoch.
        '''       
        return self.compute_prob_valid_states() 
