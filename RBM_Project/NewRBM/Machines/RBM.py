...
class RBM(object):
    ...
    def _returnNCDSteps(self,init_visible, steps, sample_last):
        '''
        ...
        if sample_last:
            ...
            #We do not sample the last step
            for _ in range(steps-1):
                aux_visible = self.sample_visible(aux_hidden)
                aux_hidden = self.sample_hidden(aux_visible)
            aux_visible = self.sample_visible(aux_hidden)
            aux_hidden = self.meanfile_hidden(aux_visible)
        
        return init_visible, init_hidden, aux_visible, aux_hidden

    def _prepareCDK(self, k, sample_last = True):
        ...
        if self.type == "exact":
            if self.computeProbs == False:
                raise Exception("If you want to compute the exact gradient you must compute probabilities!")
            ...
            init_visible, init_hidden, last_visible, last_hidden = self._returnNCDSteps(init_visible, k, sample_last = sample_last)
        ...
        if self.momentum_rate != 0:
            ...
            w_grad = self.momentum_rate * w_cumuled + learning_rate * w_grad
            b_h_grad = self.momentum_rate * b_h_cumuled + learning_rate * b_h_grad
            b_v_grad = self.momentum_rate * b_v_cumuled + learning_rate * b_v_grad

            self.f_train_CD = theano.function([],[], updates = [(self.W_params, self.W_params + w_grad),
                                                                (self.b_h, self.b_h + b_h_grad),
                                                                (self.b_v, self.b_v + b_v_grad),
                                                                (w_cumuled, w_grad),
    ...
    def _prepareProbs(self):
        ...
        self.compute_KL = theano.function([], theano.tensor.sum(self.data_probs*(theano.tensor.log(self.data_probs) - theano.tensor.log(self.probability_tensor[self.data_indexs]))))

                                
    def __init__(self, HIDDEN, VISIBLES, dataset, probabilities, learning_rate, momentum_rate = 0., sigma = 1., k = 1, type = "equal_weights", sample_last = True, computeProbs = False):
        '''
        ...
        self.computeProbs = computeProbs
        ...
        for j in range(len(just_visible_dataset)):
            ...
            for i in range(len(just_visible_dataset[j])):
                new_index = new_index * 2 + just_visible_dataset[j][-i-1]
        ...
        if computeProbs == True:
            ...
            self._prepareProbs()
        self._prepareCDK(k, sample_last=sample_last)        
