...
from Machines import Jose_RBM, Jose_RBM_2, Jose_RBM_3
from pylab import *
...
def execute(RBM_object, machine_type, k):
    ...
    print machine.compute_prob_by_state()
    print machine.data.get_value().shape
    print machine.data_indexs.get_value()

    iters = 0
    monit_likelihood = []
    while iters < NUM_EPOCHS:        
        iters += 1
        learning_algorithm(0.001, 1)
...
'''
equal_weights = [execute(Jose_RBM_3.RBM, machine_type = "equal_weights", k = k) for k in range(k_min,k_max+1)]
exact = execute(Jose_RBM_3.RBM, machine_type = "exact", k = 1)
model_weights = [execute(Jose_RBM_3.RBM, machine_type = "model_weights", k = k) for k in range(k_min,k_max+1)]
paranoia_weights = [execute(Jose_RBM_3.RBM, machine_type = "paranoia_weights", k = k) for k in range(k_min,k_max+1)]
