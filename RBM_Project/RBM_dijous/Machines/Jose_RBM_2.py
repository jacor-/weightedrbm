...
class RBM(object):
    ...
    def _prepareCDK(self, k, SEED = 4321):
        ...
        if self.type == "exact":
            ...
            momentum_update_w = momentum_rate * new_w
            momentum_update_b = momentum_rate * new_b 
            momentum_update_c = momentum_rate * new_c 
            ...
            if self.type == "equal_weights":
                new_w = theano.tensor.sum(self.data_probabilities.transpose((0,'x','x'))*(positive_phase-negative_phase), axis=0)
                new_c = theano.tensor.sum(self.data_probabilities.transpose((0,'x'))*(init_hidden - last_hidden), axis=0).transpose(('x',0))
                new_b = theano.tensor.sum(self.data_probabilities.transpose((0,'x'))*(init_visible - last_visible), axis=0).transpose(('x',0))
